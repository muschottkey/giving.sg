...
//  /* For normal swiper */
 ...
 $('.swiper-container-3').each(function(){
    ...
    $('#camp-holder').on('click','.checkbox-clone', function(){
        $('#select-org-type').modalPopover({
            target: $(this),
            placement: 'bottom'
        });

        $('#select-org-type').modalPopover('show');
    })

    $('#select-org-type').on('change','input', function(){
        console.log("clicked");
        var checked = $(this).prop('checked');
        var parent = $(this).attr('id');
        var content = $(this).next('label').text(); 
        if(checked == true){
            $('#select-org-type').find('input').prop("checked", false);
            $(this).prop("checked", true);
            $('#camp-holder').find('.checkbox-clone').addClass('to-hide');
     ...
     $('#close-orgs').on('click', function(){
        $('#camp-holder').find('.checkbox-clone.hidden').removeClass('hidden').fadeIn(400);
        $('#camp-holder').find('.checkbox-clone.to-hide').fadeOut(400).detach();
        $('#select-org-type').modalPopover('hide');
    })
...
function showSidebar(winWidth){
    ...
    $('.add-row').on("click", function(e){
        ...
        '<button href="#" class="btn btn-minimal delete-row"><i class="fa fa-trash-o"></i></button></div>';
